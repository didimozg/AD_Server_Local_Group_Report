...
function Write-ReportLogFileLine {
    ...
    try {
        ...
        if (-not $lockTaken) {
            Write-Verbose ("Failed to acquire the mutex for log file '{0}' within {1} ms." -f $ReportLogPath, $MutexTimeoutMs)
            return $false
        }
        ...
        Write-Verbose ("Failed to write a line to log file '{0}': {1}" -f $ReportLogPath, $_.Exception.Message)
        return $false
    }
    finally {
        if ($null -ne $mutex) {
