...
function Get-CimLocalGroupMembership {
    ...
    if ($stopwatch.Elapsed.TotalSeconds -ge $SlowQueryWarningSec) {
        Write-ReportLog -Message ("Server {0}, group {1}: ASSOCIATORS query took {2:N1} s." -f $ServerName, $Group.Name, $stopwatch.Elapsed.TotalSeconds) -Level 'WARN' -RuntimeContext $RuntimeContext
    }
    ...
    if ($addedRowsCount -gt 0) {
        Write-ReportLog -Message ("Server {0}, group {1}: WinNT fallback added missing members: {2}." -f $ServerName, $Group.Name, $addedRowsCount) -Level 'WARN' -RuntimeContext $RuntimeContext
    }
    ...
    foreach ($authenticationAttempt in $authenticationAttempts) {
        ...
        try {
            ...
            if ($authenticationAttempt.Label -eq 'CurrentSession' -and $null -ne $effectiveScanConfiguration.Credential) {
                Write-ReportLog -Message ("Server {0}: explicit credentials did not work, collection continued successfully under the current Windows session." -f $Server.ConnectionName) -Level 'WARN' -RuntimeContext $RuntimeContext
            }
            ...
            if ($shouldTryCurrentSession) {
                Write-ReportLog -Message ("Server {0}: explicit credentials failed ({1}). Trying the current Windows session." -f $Server.ConnectionName, $attemptMessage) -Level 'WARN' -RuntimeContext $RuntimeContext
                continue
            }
        }
        finally {
